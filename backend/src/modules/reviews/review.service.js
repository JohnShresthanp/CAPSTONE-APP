...
export const getMovieReviews = async ({ movieId, query, userId }) => {
    ...
    const reviewsWithLiked = reviews.map((review) => ({
        ...review,
        likedByUser: likedReviewIds.has(review.id)
    }));

    return getPagination({ data: reviewsWithLiked, page, limit, total });
};
