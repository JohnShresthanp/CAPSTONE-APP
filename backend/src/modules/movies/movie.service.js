...
export const getMovieById = async (id) => {
    ...
    if (movie.tmdbId && (!movie.cast || movie.cast.length === 0)) {
        try {
            movie = await syncTmdbMovieById(movie.tmdbId);
        } catch (err) {
            console.error('Failed to sync cast:', err.message);
        }
    }
...
export const updateNepaliMovie = async ({ id, body, file }) => {
    ...
    const movieData = {
        ...
        ...(file?.filename ? { posterUrl: `/uploads/posters/${file.filename}` } : {}),
...
const syncPersonCredits = async (person) => {
    if (!person.tmdbPersonId) return;

    assertTmdbConfigured();

    const [personResponse, creditsResponse] = await Promise.all([
        tmdb.get(`/person/${person.tmdbPersonId}`),
        tmdb.get(`/person/${person.tmdbPersonId}/combined_credits`)
    ]);

    const personData = mapTmdbPersonToPrisma(personResponse.data);
    ...
    const credits = [
        ...(creditsResponse.data?.cast || []).filter((c) => c.media_type === 'movie'),
        ...(creditsResponse.data?.crew || []).filter((c) => c.media_type === 'movie')
    ];

    const seen = new Set();
    const uniqueCredits = [];
    for (const credit of credits) {
        const key = `${credit.id}-${credit.credit_id || credit.character || credit.job || 'unknown'}`;
        if (seen.has(key)) continue;
        seen.add(key);
        uniqueCredits.push(credit);
    }

    await Promise.all(uniqueCredits.map(async (credit) => {
        const movie = await upsertTmdbListMovie(credit);
        if (!movie) return;

        const role = credit.credit_type === 'crew' || credit.department === 'Directing' || credit.job === 'Director'
            ? 'DIRECTOR' : 'ACTOR';
...
export const getPersonDetail = async (personId) => {
    ...
    await syncPersonCredits(person).catch((err) => console.error('Failed to sync person credits:', err.message));
...
export const getSimilarMovies = async (movieId) => {
    ...
    assertTmdbConfigured();
    const response = await tmdb.get(`/movie/${movie.tmdbId}/similar`);
    const results = response.data?.results || [];
    const synced = await Promise.all(results.slice(0, 12).map(upsertTmdbListMovie));
...
export const getTrendingNepaliMovies = async () => {
    const movies = await prisma.movie.findMany({
        where: { source: 'NEPALI' },
        orderBy: { createdAt: 'desc' },
        take: 12
