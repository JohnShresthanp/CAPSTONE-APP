...
const findExistingAuthIdentity = (username, email) => prisma.user.findFirst({
    where: {
        OR: [
            { email },
            { username }
...
const handleExistingAuthIdentity = (res, existingUser, { username, email }) => {
    if (!existingUser) {
        return false;
    }
...
export const registerUser = async (req, res) => {
    try {
        ...
        const existingUser = await findExistingAuthIdentity(username, email);

        if (handleExistingAuthIdentity(res, existingUser, { username, email })) {
            return;
        }
