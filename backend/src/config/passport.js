...
const buildUniqueUsername = async (tx, preferredUsername, fallback) => {
    const baseUsername = normalizeUsername(preferredUsername, fallback);
...
const upsertOAuthUser = async ({ provider, providerId, email, username, avatarUrl }) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        const uniqueUsername = await buildUniqueUsername(tx, username, email.split('@')[0]);
